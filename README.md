[![European Galaxy server](https://img.shields.io/badge/usegalaxy-.eu-brightgreen?logo=data:image/png;base64,)](https://usegalaxy.eu/root?tool_id=interactive_tool_divand)

# Docker image for DIVAnd - constructed for the use as Galaxy IT

[DIVAnd](https://github.com/gher-uliege/DIVAnd.jl) (Data-Interpolating Variational Analysis in n dimensions) is now available in [Galaxy](https://usegalaxy.eu/root?tool_id=interactive_tool_divand).

![DIVAnd in Galaxy](./diva.png)

### Run the image

You can start the container outside of Galaxy with:

```bash
docker run -it  -p 8888:8888  quay.io/galaxy/divand-jupyterhub:galaxy-2023-08-08t0904-bag jupyter-lab bash
```

### Configuration

This image is based on top of the fantastic work of Björn Grüning [base jupyter image](https://github.com/bgruening/docker-jupyter-notebook).

### Galaxy integration

Galaxy can run arbitrary Virtual Research Environments (VREs). In Galaxy terms, such VRE's are called "Interactive Tools", as they are using the same subsystem then normal Galaxy tools.
The only requirement is that those tools needs to run in containers and expose a port(s) to which Galaxy can redirect users. The Docker image for DIVAnd you can find in this repository.
The Galaxy tool defintion for the DIVAnd Interactive tool can be found [here](https://github.com/usegalaxy-eu/galaxy/blob/release_23.0_europe/tools/interactive/interactivetool_divand.xml).
